echo "Verificando regions com Cosmos DB para sua conta ..."
...
check_region() {
  ...
  result=$(az cosmosdb list-region-capacity \
    --location "$region" \
    --query "[?status=='Available']" \
    -o tsv 2>/dev/null)

  if [ -n "$result" ]; then
    echo "Cosmos DB disponível: $region"
  else
    echo "Cosmos DB indisponível: $region"
  fi
}
